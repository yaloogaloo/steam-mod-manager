"""OfflineProvider routing — Steam archive / Nexus manual / GitHub snapshot."""
...
def test_provider_names_by_platform() -> None:
    assert SteamOfflineProvider().get_provider_name() == PROVIDER_STEAM_ARCHIVE
    assert NexusManualOfflineProvider().get_provider_name() == PROVIDER_NEXUS_MANUAL_IMPORT
...
def test_nexus_manual_and_github_snapshot(
    tmp_path: Path,
    db: DatabaseManager,
) -> None:
    ...
    class FakeLayout:
        def snapshot(self, url: str, output_dir: Path | str) -> LayoutSnapshotResult:
            ...
            return LayoutSnapshotResult(
                success=True, html_path=index, backend="layout", asset_count=0
            )

    nr = NexusManualOfflineProvider().import_offline_page(
        nexus.mod_id, html, managed_path=nexus_folder, library_root=lib
    )
    ...
    assert nr.provider == PROVIDER_NEXUS_MANUAL_IMPORT
    ...
    assert "nexus-local" in nr.index_path.read_text(encoding="utf-8")
