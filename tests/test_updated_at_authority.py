...
def _seed(
    db: DatabaseManager, workshop: int = 91001, *, updated_at: str | None = None
) -> tuple[str, str]:
    """Create Steam entity; return ``(updated_at stamp, mods.mod_id PK)``."""
    stamp = updated_at or _iso(1)
    created = create_steam_test_mod(
        db, external_id=str(workshop), title="AuthMod", app_id=42, game_name="AuthGame"
    )
    ...
    db._conn.execute(
        ...
        "WHERE mod_id = ?",
        (stamp, f"/tmp/{workshop}", CONTENT_HEALTHY, IDENTITY_STATUS_OK, int(pk)),
    )
    db._conn.commit()
    return stamp, pk


def _read_updated_at(db: DatabaseManager, pk: str | int) -> str:
...
def test_touch_mod_updated_at_requires_legal_reason(db: DatabaseManager) -> None:
    before, pk = _seed(db, 91010)
    with pytest.raises(UpdatedAtAuthorityError):
        db.touch_mod_updated_at(pk, reason="recovery")
    assert _read_updated_at(db, pk) == before
    with pytest.raises(UpdatedAtAuthorityError):
        db.touch_mod_updated_at(pk, reason="")
    db.touch_mod_updated_at(pk, reason="user_edit")
    assert _read_updated_at(db, pk) != before
...
def test_status_recovery_peel_does_not_touch_updated_at(db: DatabaseManager) -> None:
    ...
    db._conn.execute(
        ...
        ("identity_conflict", "identity_conflict", int(pk)),
    )
    db._conn.commit()
...
def test_update_mod_status_scan_does_not_touch(db: DatabaseManager) -> None:
    before, pk = _seed(db, 91302)
    db.update_mod_status(pk, invalid=True, invalid_reason="scan", touch_check_time=True)
    assert _read_updated_at(db, pk) == before


def test_offline_backup_deploy_do_not_touch(db: DatabaseManager) -> None:
    before, pk = _seed(db, 91401)
    db.update_mod_offline_status(pk, status="ready", provider="nexus")
    assert _read_updated_at(db, pk) == before
    ...
    assert _read_updated_at(db, pk) == before
    db.update_mod_deploy_status(pk, deploy_status="deployed", deploy_path="/game/x")
    assert _read_updated_at(db, pk) == before
    ...
    assert _read_updated_at(db, pk) == before
    db.update_mod_platform_info(pk, title="RefreshedTitle")
    assert _read_updated_at(db, pk) == before


def test_upsert_mod_conflict_does_not_bump(db: DatabaseManager) -> None:
    workshop = 91402
    before, pk = _seed(db, workshop, updated_at=_iso(50))
    db.upsert_mod(
        ModMetadata(published_file_id=str(workshop), title="NewSteamTitle", app_id=42)
    )
    assert _read_updated_at(db, pk) == before
...
def test_user_enable_and_conflict_annotation_touch(db: DatabaseManager) -> None:
    ...
    db._conn.commit()
    db.update_mod_conflict_annotation(pk, conflict=True, note="user mark")
    assert _read_updated_at(db, pk) != older


def test_user_tag_touches_updated_at(db: DatabaseManager) -> None:
    before, pk = _seed(db, 91503)
    from core.db_manager import TAG_TYPE_CATEGORY

    db.add_mod_tag(pk, TAG_TYPE_CATEGORY, "rpg")
    assert _read_updated_at(db, pk) != before
...
def test_db_manager_content_status_default_is_false() -> None:
    """Default must be False in source (pytest may wrap the live method)."""
    ...
    for node in tree.body:
        ...
        for item in node.body:
            if not isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                continue
            if item.name != "update_mod_content_status":
                continue
            for arg, default in zip(
                ...
                if arg.arg == "touch_updated_at":
                    assert isinstance(default, ast.Constant) and default.value is False
                    return
    raise AssertionError("touch_updated_at default not found on update_mod_content_status")
