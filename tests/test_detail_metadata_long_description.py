...
def _seed(
    ...
    *,
    ...
    create_steam_test_mod(db, external_id=mid, title=title, app_id=100, game_name="SomeGame")
...
def test_long_description_survives_panel_resize(
    qapp: QApplication, tmp_path: Path, db: DatabaseManager, monkeypatch
) -> None:
    monkeypatch.setattr("ui.mod_detail_panel.get_db", lambda: db)
    library = tmp_path / "library"
