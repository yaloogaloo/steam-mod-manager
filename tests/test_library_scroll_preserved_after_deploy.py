"""Library scroll position stays stable after deploy refresh."""
...
def test_library_scroll_preserved_after_deploy(
    ...
    # Force layout so the flow has height / scroll range.
    view.library_host.adjustSize()
    view.scroll.updateGeometry()
    _pump()

    if bar.maximum() <= 0:
        # Fallback: still verify capture/restore helpers round-trip.
    ...
    # Must not jump to the absolute bottom from a mid-list position.
    assert after < bar.maximum() or bar.maximum() == 0
    if before > 20:
        assert after > 0
    # Target mod must remain findable / selected after deploy refresh.
    focused = view._card_for_mod_id("9010")
    assert focused is not None
    assert view._selected_card is focused or view._selected_path == focused.managed_path
