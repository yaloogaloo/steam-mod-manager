...
def _seed(
    ...
    *,
    ...
    folder = library / 'Game' / f'Mod{external_id}'
    ...
    created = create_steam_test_mod(
        ...
        title=title or f'M{external_id}',
        app_id=app_id,
        game_name=game_name,
    )
...
def test_content_missing_does_not_produce_conflict(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    folder, pk = _seed(
        library, db, external_id='701', with_payload=False, title='Empty',
        app_id=424242, game_name='Game',
    )
    ...
    persist_evaluated_content_status(pk, folder, db=db, folder_present=True, metadata_missing=False, sync_sticky_marker=True)
...
def test_identity_conflict_does_not_write_user_conflict(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    db.update_game_deploy_config(424242, name='Game')
    folder, pk = _seed(
        library, db, external_id='703', title='Id', app_id=424242, game_name='Game',
    )
    db.update_mod_identity_fields(pk, identity_status=CONTENT_IDENTITY_CONFLICT)
...
def test_user_annotation_is_only_writer_roundtrip(tmp_path: Path, db: DatabaseManager) -> None:
    library = tmp_path / 'mod'
    folder, pk = _seed(library, db, external_id='704', title='User')
    db.update_mod_status(pk, invalid=False, touch_check_time=True)
    assert db.get_mod_status(pk).conflict_status == CONFLICT_STATUS_NONE
    set_conflict_annotation(pk, note='from detail', db=db)
    assert db.get_mod_status(pk).conflict_status == CONFLICT_STATUS_CONFLICT
