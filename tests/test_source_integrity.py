...
def db(tmp_path: Path) -> DatabaseManager:
    ...
    manager = DatabaseManager.instance(tmp_path / "source_integrity.db")
...
def test_case1_rename_archive_hash_match_auto_fixes(
    ...
    _set_files(db, pk, [_entry(path="original.zip")])

    result = reconcile_source(pk, managed_path=mod, db=db)
    assert result.updated
    assert result.auto_fixed == ["renamed.zip"]
    assert db.get_mod_files(pk).files[0].path == "renamed.zip"
    validate_source(pk, managed_path=mod, db=db)
...
def test_case2_move_archive_hash_match_auto_fixes(
    ...
    _set_files(db, pk, [_entry(path="subdir/old.zip", filename="old.zip")])

    result = reconcile_source(pk, managed_path=mod, db=db)
    assert result.updated
    assert result.auto_fixed == ["at-root.zip"]
    assert db.get_mod_files(pk).files[0].path == "at-root.zip"
...
def test_case3_changed_hash_updates_metadata(
    ...
    _set_files(db, pk, [entry])

    _write_zip(mod / "mod.zip", data=b"version-two-changed")

    out = validate_source(pk, managed_path=mod, db=db)
    assert "mod.zip" in out.source_changed
    stored = db.get_mod_files(pk).files[0].metadata[META_CONTENT_HASH]
...
def test_case4_multiple_versions_user_selection_not_auto_pick(
    ...
    _set_files(db, pk, [selected, other])

    resolved = resolve_deploy_source(pk, managed_path=mod, db=db)
...
def test_case5_empty_zip_rejected(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    with zipfile.ZipFile(empty, "w"):
        pass
    _set_files(db, pk, [_entry(path="empty.zip")])
...
def test_case7_history_version_excluded(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    _set_files(db, pk, [_entry(path="old.zip")])
...
def test_case8_import_cache_excluded(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    _set_files(db, pk, [_entry(path="cached.zip")])
...
def test_case9_direct_deploy_without_refresh(
    ...
    _set_files(db, pk, [_entry(path="stale.zip")])
