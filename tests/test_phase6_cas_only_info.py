...
def test_open_uses_offline_view_not_info_assets(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    store_root = tmp_path / "store"
    ...
    assert finalize_live_offline_to_cas(mod, store=AssetStore(root=store_root)).ok
    assert not (mod / ".info" / "assets").exists()
    ...
    # LIVE durable tree still has no assets
    assert not (mod / ".info" / "assets").exists()
...
def test_missing_cas_repair_fails(
    ...
    store.delete(digest)
    repair = repair_info_assets_from_backup_store(mod, mod_id="11", store=store)
...
def test_legacy_fallback_open_without_manifest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Phase 11: leftover ``.info/assets`` is not an OPEN source."""
    store_root = tmp_path / "store"
    monkeypatch.setattr("core.paths.asset_store_dir", lambda: store_root)
    mod = _steam_mod(tmp_path / "M", {"a.png": TINY_A})
    opened = ensure_live_offline_openable(mod)
