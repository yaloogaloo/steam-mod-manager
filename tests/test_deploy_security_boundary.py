...
# ---------------------------------------------------------------------------
# Case1 â€?malicious manifest target traversal
# ---------------------------------------------------------------------------
...
def test_case1_malicious_manifest_target_traversal(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    assert "å®‰å…¨æ ¡éªŒ" in str(out.get("error") or "") or "mismatch" in str(
        out.get("error") or ""
    ).lower() or "æ¸…å•" in str(out.get("error") or "")


# ---------------------------------------------------------------------------
# Case2 â€?illegal backup path restore
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Case3 â€?Mod A must not use Mod B manifest
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Case4 â€?remove_empty_parents never deletes protected roots
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Case5 â€?source outside workspace rejected
# ---------------------------------------------------------------------------
...
def test_case5_source_outside_workspace(tmp_path: Path, db: DatabaseManager) -> None:
    ...
    assert out["success"] is False
    assert "å®‰å…¨æ ¡éªŒ" in str(out.get("error") or "")


# ---------------------------------------------------------------------------
# Case6 â€?shared / referenced backup prune protection
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Case7 â€?illegal manifest refuses undeploy (no deletes)
# ---------------------------------------------------------------------------
