...
def _seed_mod(
    ...
    *,
    ...
    """Create Entity + managed folder. Returns (folder, mods.mod_id PK)."""
    ...
    created = create_steam_test_mod(
        ...
        app_id=ANNO_APP_ID,
        game_name=game,
        source_url=f"https://example.com/{workshop_id}",
    )
    ...
    extra: dict = {
        ...
        "author": f"author-{workshop_id}",
        "source_type": "nexus",
        "url": f"https://example.com/{workshop_id}",
        "source_url": f"https://example.com/{workshop_id}",
        "published_file_id": workshop_id,
        "offline_status": "generated" if with_offline else "",
    }
    write_info_sidecar(
        folder,
        internal_id=frozen,
    ...
    if with_cover:
        ...
        cover.write_bytes(f"cover-{workshop_id}".encode("utf-8"))
        data = json.loads((info / METADATA_FILENAME).read_text(encoding="utf-8"))
        data["cover_path"] = ".info/cover.jpg"
    ...
    if with_offline:
        ...
        (offline / "index.html").write_text(
            f"<html>offline-{workshop_id}</html>", encoding="utf-8"
        )
        data = json.loads((info / METADATA_FILENAME).read_text(encoding="utf-8"))
    ...
    bind_managed_path(db, pk, folder, game_name=game, title=name)
    if favorite:
...
def test_1_delete_mod_still_visible_from_backup(
    ...
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="Mod A", workshop_id="950001"
    )
    shutil.rmtree(folder)
    reconcile_folder_presence(library)

    visible = list_visible_mods(library)
    assert pk in {m.internal_id for m in visible}
    assert "ws-950001" in {m.workspace_id for m in visible}
...
def test_2_delete_entire_game_keeps_game_and_mods(
    ...
    library = tmp_path / "mod"
    _, pk_a = _seed_mod(
        library, db, game="Anno 1800", title="Mod A", workshop_id="950002"
    )
    _, pk_b = _seed_mod(
        library, db, game="Anno 1800", title="Mod B", workshop_id="950003"
    )
    _, pk_c = _seed_mod(
        library, db, game="Anno 1800", title="Mod C", workshop_id="950004"
    )
    ...
    assert {m.internal_id for m in visible} == {pk_a, pk_b, pk_c}
    assert {m.workspace_id for m in visible} == {
        "ws-950002",
        "ws-950003",
        "ws-950004",
    }
...
def test_3_delete_game_detail_resolves_all_mods(
    ...
    library = tmp_path / "mod"
    paths = [
        _seed_mod(library, db, game="Anno 1800", title="Mod A", workshop_id="950010"),
        _seed_mod(library, db, game="Anno 1800", title="Mod B", workshop_id="950011"),
    ]
    shutil.rmtree(library / "Anno 1800")
    reconcile_folder_presence(library)

    for path, pk in paths:
        resolved = resolve_mod_metadata(pk, path)
        assert resolved is not None
        assert resolved.folder_present is False
...
def test_4_delete_game_cover_from_backup(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="CoverMod", workshop_id="950020"
    )
    shutil.rmtree(library / "Anno 1800")
    reconcile_folder_presence(library)
...
def test_5_delete_game_offline_from_backup(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="OffMod", workshop_id="950021"
    )
    shutil.rmtree(library / "Anno 1800")
    reconcile_folder_presence(library)

    offline = resolve_offline_page(pk, folder)
    assert offline is not None
    assert offline.is_file()
    assert "offline_view" in str(offline).replace("\\", "/")
    assert "offline-950021" in offline.read_text(encoding="utf-8")

    snap = build_library_snapshot(library)
    card = next(c for c in snap.cards if c.id == pk)
...
def test_6_restore_game_info_overwrites_backup(
    ...
    reconcile_folder_presence(library)
    resolved = resolve_mod_metadata(pk, restored)
    assert resolved is not None
...
def test_7_backup_beats_sqlite_when_folder_missing(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="Pri", workshop_id="950040"
    )
    # SQLite deliberately has OLD display fields (no mint / no Workshop-as-PK).
...
def test_8_backup_never_writes_info(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="Safe", workshop_id="950050"
    )
...
def test_9_detail_resolve_does_not_sync(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="NoSync", workshop_id="950060"
    )
    calls: list[str] = []
    monkeypatch.setattr(
    ...
    resolve_mod_metadata(pk, folder)
    resolve_cover_path(pk, folder)
    resolve_offline_page(pk, folder)
    assert calls == []
...
def test_10_one_metadata_change_one_sync(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="Once", workshop_id="950070"
    )
    calls: list[str] = []
    real = sync_after_metadata_change

    def tracking(mod_id, managed_path, reason, **kwargs):
        calls.append(str(reason))
        return real(mod_id, managed_path, reason, wait=True, **kwargs)
    ...
    assert calls == ["edit"]
    assert load_backup(pk).metadata.get("title") == "Once-B"
...
def test_favorite_preserved_when_missing(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="Fav", workshop_id="950080", favorite=True
    )
    shutil.rmtree(folder)
    reconcile_folder_presence(library)
    resolved = resolve_mod_metadata(pk, folder)
...
def test_info_cover_missing_falls_back_to_backup_before_sync(
    ...
    library = tmp_path / "mod"
    folder, pk = _seed_mod(
        library, db, game="Anno 1800", title="Cov", workshop_id="950090"
    )
...
def test_game_derived_from_backup_without_games_table_row(
    ...
    bind_managed_path(db, pk, folder, game_name=game, title=title)
    sync_after_metadata_change(pk, folder, "import", wait=True)
    ...
    assert any(m.internal_id == pk for m in visible)
    assert any(m.workspace_id == "ws-950100" for m in visible)
