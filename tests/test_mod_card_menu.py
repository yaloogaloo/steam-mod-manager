...
def _card_parent_host(view: ModLibraryView):
    return view._cards_host if view._cards_host is not None else view.library_host
...
def test_library_refresh_does_not_leave_menu_windows(
    ...
    for i in range(4):
        pub = str(95000 + i)
        _seed_mod(db, lib, pub_id=pub, title=f"Menu Mod {i}")
    ...
    for card in view._cards:
        assert card.parent() is _card_parent_host(view)
        assert not any(
