...
def db(tmp_path: Path) -> DatabaseManager:
    ...
    manager = DatabaseManager.instance(tmp_path / "backup_restore.db")
    yield manager
    manager.close()
    DatabaseManager.reset_instance()
...
def _add_mod(
    ...
    *,
    ...
    prove_managed_folder(db, mod, handle=pk, title=title, app_id=app_id, game_name="SomeGame")
    return mod, pk
...
def test_case2_restore_game_original(tmp_path: Path, db: DatabaseManager) -> None:
    library = tmp_path / "mod"
    mods_root = _setup_game(db, tmp_path)
    source, pk = _add_mod(library, db, mid="92002", title="OverwriteMe")
...
def test_case4_deploy_failure_auto_restore(tmp_path: Path, db: DatabaseManager) -> None:
    library = tmp_path / "mod"
    mods_root = _setup_game(db, tmp_path)
    _source, pk = _add_mod(library, db, mid="92004", title="FailMod")
...
def test_case5_legacy_manifest_without_backup(tmp_path: Path, db: DatabaseManager) -> None:
    library = tmp_path / "mod"
    mods_root = _setup_game(db, tmp_path)
    source, pk = _add_mod(library, db, mid="92005", title="Legacy")
...
def test_case6_two_mods_same_target_conflict_unchanged(
    ...
    library = tmp_path / "mod"
    mods_root = _setup_game(db, tmp_path)
    a, pk_a = _add_mod(library, db, mid="92006", title="ModA", files={"shared.txt": "A"})
    b, pk_b = _add_mod(library, db, mid="92007", title="ModB", files={"shared.txt": "B"})
    ...
    assert overwrite
    assert sorted(overwrite[0].mods) == sorted([pk_a, pk_b])
